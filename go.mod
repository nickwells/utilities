module github.com/nickwells/utilities

go 1.26.0

require (
	github.com/nickwells/check.mod/v2 v2.1.27
	github.com/nickwells/cli.mod v1.1.11
	github.com/nickwells/dirsearch.mod/v2 v2.0.25
	github.com/nickwells/english.mod v1.2.7
	github.com/nickwells/errutil.mod v1.2.22
	github.com/nickwells/filecheck.mod v1.2.11
	github.com/nickwells/gogen.mod v1.11.36
	github.com/nickwells/location.mod v1.2.34
	github.com/nickwells/mathutil.mod/v2 v2.5.8
	github.com/nickwells/snippet.mod v1.2.18
	github.com/nickwells/testhelper.mod/v2 v2.4.3
	github.com/nickwells/timer.mod v1.2.6 // indirect
	github.com/nickwells/twrap.mod v1.5.12
	github.com/nickwells/units.mod/v2 v2.1.23
	github.com/nickwells/unitsetter.mod/v4 v4.2.44
	github.com/nickwells/verbose.mod v1.1.17
	github.com/nickwells/versionparams.mod v1.2.21
	github.com/nickwells/xdg.mod v1.0.10
	golang.org/x/sys v0.42.0
)

require (
	github.com/nickwells/col.mod/v6 v6.0.2
	github.com/nickwells/groupsetter.mod v1.0.1
	github.com/nickwells/param.mod/v7 v7.0.1
	github.com/nickwells/tempus.mod v1.2.9
)

require github.com/nickwells/strdist.mod/v2 v2.1.0 // indirect

require (
	github.com/nickwells/fileparse.mod v1.1.37 // indirect
	github.com/nickwells/pager.mod v1.0.16 // indirect
	golang.org/x/exp v0.0.0-20260218203240-3dfff04db8fa // indirect
	golang.org/x/term v0.40.0 // indirect
)
